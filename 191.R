...
turnUpNull <- function(dat){
  ...
  #   dat[1] <- dat[2]
  # }else{
  #   if(a3[length(a3)] == length(dat)){
  #     dat[a3[-length(a3)]] <- (dat[a3[-length(a3)]-1]+dat[a3[-length(a3)]+1])/2
  #     dat[a3[length(a3)]] <- dat[length(dat)-1]
  #   }else{
  #     dat[a3] <- (dat[a3-1]+dat[a3+1])/2
  #   }
  # }
...
mu_index <- function(dat,LA=50){
  ...
  ro <- c(24158, 24508, 25848, 26887, 27392,
          28084, 28167, 28426, 29035,
          29092, 30137, 30339, 31284,
          31616, 32477, 32941)
  
  mean_z <- c(351, 1340, 1039, 505,
              692, 83, 259, 609, 57,
              1045, 202, 945, 332, 861, 464)
  
  var_sig <- c(60, 14, 12, 16, 42, 5792, 8977,
               13444, 8806, 9111, 16902, 15429, 51, 24, 58)
...
create_list<- function(len_vec = 4){
  df <- list("Ts"=rep(0,len_vec),
                   "Tm"=rep(0,len_vec),
                   "NOR"=rep(0,len_vec),
                   "HOP"=rep(0,len_vec),
                   "MOR"=rep(0,len_vec),
                   "C31S_HOP"=rep(0,len_vec),
                   "C31R_HOP"=rep(0,len_vec),
                   "GAM"=rep(0,len_vec),
                   "C32S_HOP"=rep(0,len_vec),
                   "C32R_HOP"=rep(0,len_vec),
                   "C33S_HOP"=rep(0,len_vec),
                   "C33R_HOP"=rep(0,len_vec),
                   "C34S_HOP"=rep(0,len_vec),
                   "C34R_HOP"=rep(0,len_vec),
                   "C35S_HOP"=rep(0,len_vec),
                   "C35R_HOP"=rep(0,len_vec),
             "Ts_Tm" = 0,
             "Ts_Ts_Tm" = 0,
             "NOR_HOP" = 0,
             "MOR_MOR_HOP" = 0,
             "C31S_HOP_C31S_HOP_C31R_HOP" = 0,
             "SUM_C31_C35_HOP" = 0)
...
area_percent_peak <- function(text, name_well, file_error){
  ...
  listRes <- create_list()
      ...
      tryCatch(
        error = function(cnd) {
          ...
          print(msg)
          vec_error_output <<- c(vec_error_output,paste("OIL 191mz",name_well, msg, sep = ": "))
        },
        {
  ...
  listRes$Ts_Tm <- listRes$Ts[1]/listRes$Tm[1] 
  listRes$Ts_Ts_Tm <- listRes$Ts[1]/(listRes$Ts[1]+listRes$Tm[1])
  listRes$NOR_HOP <- listRes$NOR[1]/listRes$HOP[1] 
  listRes$MOR_MOR_HOP <- listRes$MOR[1]/(listRes$MOR[1]+listRes$HOP[1])
  listRes$C31S_HOP_C31S_HOP_C31R_HOP <- listRes$C31S_HOP[1]/(listRes$C31S_HOP[1]+listRes$C31R_HOP)
  listRes$SUM_C31_C35_HOP <- (listRes$C31S_HOP[1]+listRes$C31R_HOP[1]+
                                     listRes$C32S_HOP[1]+listRes$C32R_HOP[1]+
                                     listRes$C33S_HOP[1]+listRes$C33R_HOP[1]+
                                     listRes$C34S_HOP[1]+listRes$C34R_HOP[1]+
                                     listRes$C35S_HOP[1]+listRes$C35R_HOP[1])/listRes$HOP[1] 
